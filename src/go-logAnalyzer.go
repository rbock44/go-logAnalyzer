...
func main() {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "## "+logAnalyzer.REGEX_LEVEL_DUPLICATE+"-SECTION") {
			regexName = strings.TrimSpace(strings.TrimPrefix(line, "## "+logAnalyzer.REGEX_LEVEL_DUPLICATE+"-SECTION"))
	...
	ignoredSrcFilesRegEx := make([]string, 0)
	...
	if *ignoreSrcFilesFileUrl != "" {
		...
		for scanner.Scan() {
			...
			if !strings.HasPrefix(line, "##") && line != "" && line != "\n" {
				...
				ignoredSrcFilesRegEx = append(ignoredSrcFilesRegEx, line)
			}
		}
	...
	if fileStat.IsDir() {
		...
		for _, childName := range files {
			...
			logFileIsOk, logMsg := processLogFile(regularExpressions, ignoredSrcFilesRegEx, *duplicateBufferSize, filepath.Join(fileToAnalyze.Name(), childName), *verbose, childOutFileName)
		...
		logFileIsOk, logMsg := processLogFile(regularExpressions, ignoredSrcFilesRegEx, *duplicateBufferSize, fileToAnalyze.Name(), *verbose, *outFileUrl)
...
func processLogFile(regularExpressions []logAnalyzer.NamedRegEx, ignoredSrcFiles []string, duplicateBufferSize int, logFileUrl string, verbose bool, outFileUrl string) (logFileIsOk bool, logMsg string) {
	...
	logFileIsOk, hits = logAnalyzer.IsFileOK(regularExpressions, ignoredSrcFiles, duplicateBufferSize, logFileUrl)
...
func writeErrorFile(errorFileUrl string, hits []string) {
	...
	if errorFileUrl != "" {
		...
		if _, err := os.Stat(outFileUrlInternal); err == nil {
			fileNameParts := strings.Split(outFileUrlInternal, ".")
			...
			for i, part := range fileNameParts {
				if i == len(fileNameParts)-1 {
					now := time.Now().Local()
					newOutFileName = newOutFileName + "_" + now.Format(time.RFC3339)
				}
			...
			outFileUrlInternal = newOutFileName
